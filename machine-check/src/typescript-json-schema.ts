...
function addSimpleType(def: Definition, type: string): boolean {
  for (const k in def) {
    if (!Object.getOwnPropertyDescriptor(simpleTypesAllowedProperties, k)) {
      return false
    }
  }
...
function makeNullable(def: Definition): Definition {
  if (!addSimpleType(def, 'null')) {
    ...
    if (union) {
      ...
      for (const k in def) {
        const descr = Object.getOwnPropertyDescriptor(def, k)
        if (descr !== undefined) {
          Object.defineProperty(subdef, k, descr)
          Reflect.deleteProperty(def, k)
        }
      }
 ...
 */
// prettier-ignore
const validationKeywords: {[k:string]:true|undefined} = {
    multipleOf: true,               // 6.1.
    maximum: true,                  // 6.2.
...
const subDefinitions: { [k: string]: true | undefined } = {
  items: true,
...
export class JsonSchemaGenerator {
  ...
  private parseCommentsIntoDefinition(
    ...
    otherAnnotations: { [k: string]: true | undefined },
  ): void {
    if (!symbol) {
      return
    }
    ...
    jsdocs.forEach((doc) => {
      ...
      if (name.startsWith('TJS-')) {
        ...
        const match: string[] | RegExpExecArray | null = new RegExp(REGEX_TJS_JSDOC).exec(
          originalText,
        )
        if (match) {
          name = match[1]
      ...
      const defn = definition as { [k: string]: unknown }
      ...
      if (subDefinitions[name]) {
        ...
        if (match) {
          const k = match[1]
          const v = match[2]
          defn[name] = { ...(defn[name] as object), [k]: v ? parseValue(symbol, k, v) : true }
          return
        }
      }
      ...
      if (validationKeywords[name] || this.userValidationKeywords[name]) {
        defn[name] = text === undefined ? '' : parseValue(symbol, name, text)
      } else {
        // special annotations
  ...
  private getDefinitionForProperty(prop: ts.Symbol, node: ts.Node): Definition | null {
    ...
    if (Object.prototype.hasOwnProperty.call(definition, 'ignore')) {
      return null
    }
  ...
  private getUnionDefinition(
    ...
    if (schemas.length === 1) {
      for (const k in schemas[0]) {
        const descr = Object.getOwnPropertyDescriptor(schemas[0], k)
        if (descr !== undefined) {
          Object.defineProperty(definition, k, descr)
        }
      }
    } else {
      ;(definition as { [k: string]: unknown })[unionModifier] = schemas
    }
    return definition
  }
      ...
      if (this.args.required) {
        const requiredProps = props.reduce((required: string[], prop: ts.Symbol) => {
          ...
          if (
            ...
            !Object.prototype.hasOwnProperty.call(def, 'ignore')
          ) {
  ...
  private getTypeDefinition(
    ...
    if (prop) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(defs, 'ignore') ||
        Object.prototype.hasOwnProperty.call(defs, 'type')
      ) {
        return defs
      }
    }
    ...
    // Parse comments
    const otherAnnotations: { [k: string]: true | undefined } = {}
  ...
  public getSchemaForType(typ: ts.Type) {
    const KEY = '01234\u001e'
    this.allSymbols[KEY] = typ
    return this.getSchemaForSymbol(KEY)
  }
  ...
  public getSchemaForSymbols(
    ...
    const root: Definition & { definitions: { [k: string]: DefinitionOrBoolean } } = {
...
export function buildGenerator(
  ...
  for (const pref in args) {
    const descr = Object.getOwnPropertyDescriptor(args, pref)
    if (descr !== undefined) {
      Object.defineProperty(settings, pref, descr)
    }
  }
  ...
  if (args.out) {
    return new Promise((resolve, reject) => {
      ...
      fs.mkdir(path.dirname(args.out), { recursive: true }, function (mkErr: Error) {
        if (mkErr) {
          return reject(
            new Error('Unable to create parent directory for output file: ' + mkErr.message),
          )
        }
